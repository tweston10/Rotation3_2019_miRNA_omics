...
rows_to_keep.CorF2 <- which(colnames(Corr.weight.F) %in% Corr.WGCNA2)

Corr.weight.Mat <- Corr.weight.F[,rows_to_keep.CorF]
Corr.weight.Mat2 <- Corr.weight.F[,rows_to_keep.CorF2]

write.csv(Corr.weight.Mat,"Weighted_Spearman_WGCNA.csv")
write.csv(Corr.weight.Mat2,"Weighted_Spearman_WGCNA2.csv")

data <- read.csv("Weighted_Spearman_WGCNA.csv", row.names=1,header=TRUE)
par(cex=0.8)
corrplot(as.matrix(data), tl.col="black",tl.srt=45,insig = "blank")

data <- read.csv("Weighted_Spearman_WGCNA3.csv", row.names=1,header=TRUE)
par(cex=0.8)
corrplot(as.matrix(data), tl.col="black",tl.srt=45,insig = "blank")
